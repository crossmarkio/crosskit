...
const Index = () => {
  const [array, setArray] = useState([
    <Input title="bulk" id={"0-input"} submit />,
  ]);
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    let txns = array.map((_input, index) => {
      return e.target[`${index}-input`].value;
    });
    let address = repo.General.getAddress();

    if (address) await sdk.bulkSubmitAndWait(address, txns);
  };
  ...
  return (
    ...
    >
      <TitleSection title="bulk submit" />
        ...
        <div
          ...
          onClick={() => {
            setArray((prev) => {
              ...
              array.push(<Input title="bulk" id={`${length}-input`} submit />);
              return array;
            });
          }}
        >
